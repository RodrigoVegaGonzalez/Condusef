import { ESTATUS_TICKET, NOMBRE_MESES, ID_MESES, QUEJAS_FICTICIAS } from "../constantes.js";
...
$("#btn_agregarDatosFicticios").on('click', function () {
    ...
    $.each(QUEJAS_FICTICIAS, function (index, queja) {
        queja.QuejasDenominacion = $("#institucion-clave").val();
        queja.QuejasSector = $("#sector").val();
        queja.QuejasFolio = generarFolio();
        tabla.row.add(queja).draw();
    });
})
...
$("#modalAgregarQueja").on('shown.bs.modal', function () {
...
$('input[name=QuejasTipoPersona]').on('change', function () {
    if ($('input[name=QuejasTipoPersona]:checked').val() === "2") {
        ...
        $('#QuejasEdad').prop('disabled', true);
    } else {
        $('input[name=QuejasSexo]').prop('disabled', false);
        $('#QuejasEdad').prop('disabled', false);
    }
});

$('input[name=QuejasEstatus]').on('change', function () {
    if ($('input[name=QuejasEstatus]:checked').val() === "1") {
        $("#QuejasFecResolucion").prop('disabled', true);
        $("#QuejasFecNotificacion").prop('disabled', true);
        $("#QuejasRespuesta").prop('disabled', true);
    } else {
        $("#QuejasFecResolucion").prop('disabled', false);
        $("#QuejasFecNotificacion").prop('disabled', false);
        $("#QuejasRespuesta").prop('disabled', false);
    }
});
...
function limpiarAgregarQueja() {
    ...
    $('input[name=QuejasEstatus][value=2]').prop('checked', true);
...
function obtenerPeriodoReportar() {
    ...
    cambiarValorSelect("QuejasNoMes", mesActual.toString());
